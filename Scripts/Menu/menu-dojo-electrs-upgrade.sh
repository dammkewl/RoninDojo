...
# using the backslash \ along with sed insert command so that the spaces are not ignored
sudo sed -i '339i \  if [ "$INDEXER_INSTALL" == "on" ]; then' ~/dojo/docker/my-dojo/dojo.sh
sudo sed -i '340i \    V3_ADDR_ELECTRS=$( docker exec -it tor cat /var/lib/tor/hsv3electrs/hostname )' ~/dojo/docker/my-dojo/dojo.sh
sudo sed -i '341i \    echo "Electrs hidden service address (v3) = $V3_ADDR_ELECTRS"' ~/dojo/docker/my-dojo/dojo.sh
sudo sed -i '342i \  fi' ~/dojo/docker/my-dojo/dojo.sh
sudo sed -i '343G' ~/dojo/docker/my-dojo/dojo.sh
