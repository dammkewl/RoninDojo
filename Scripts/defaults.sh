...
# whirlpool
if sudo test -f /var/lib/docker/volumes/my-dojo_data-whirlpool/_data/.whirlpool-cli/whirlpool-cli-config.properties; then
    WHIRLPOOL_API_KEY=$(sudo grep cli.apiKey /var/lib/docker/volumes/my-dojo_data-whirlpool/_data/.whirlpool-cli/whirlpool-cli-config.properties | cut -d '=' -f2)
fi
...
if sudo test -d /var/lib/docker/volumes/my-dojo_data-tor/_data/hsv3dojo; then
    V3_ADDR_API=$(sudo cat /var/lib/docker/volumes/my-dojo_data-tor/_data/hsv3dojo/hostname)
fi

# whirlpool
if sudo test -d /var/lib/docker/volumes/my-dojo_data-tor/_data/hsv3whirlpool; then
    V3_ADDR_WHIRLPOOL=$(sudo cat /var/lib/docker/volumes/my-dojo_data-tor/_data/hsv3whirlpool/hostname)
fi

# explorer
if sudo test -d /var/lib/docker/volumes/my-dojo_data-tor/_data/hsv3explorer; then
    V3_ADDR_EXPLORER=$(sudo cat /var/lib/docker/volumes/my-dojo_data-tor/_data/hsv3explorer/hostname)
fi

# electrum server
if sudo test -d /var/lib/docker/volumes/my-dojo_data-tor/_data/hsv3electrs; then
    V3_ADDR_ELECTRS=$(sudo cat /var/lib/docker/volumes/my-dojo_data-tor/_data/hsv3electrs/hostname)
fi
